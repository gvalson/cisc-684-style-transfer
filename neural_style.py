...
parser.add_argument("-content_weight", type=float, default=8e0)
...
parser.add_argument("-output_image", default='outputs/out.png')
...
def main():
    ...
    if params.load_gram:
        l = 0
        gram = np.load(params.gram_file).mean(axis=0)
        gram = torch.from_numpy(gram).cuda()
        for j in style_losses:
            l_ = l + j.target.nelement()
            j.target = gram[l: l_].view(j.target.shape)
            l = l_
